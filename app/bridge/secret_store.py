...
class SecretStore(QObject):
    ...
    def _read_vault(self) -> dict[str, Any]:
        if not self._vault_file.exists() or self._vault_file.stat().st_size == 0:
            return {}
        try:
            payload = self._ensure_fernet().decrypt(self._vault_file.read_bytes())
    ...
    def _write_vault(self, data: dict[str, Any]) -> None:
        ...
        token = self._ensure_fernet().encrypt(payload)
    ...
    def _ensure_fernet(self):
        if self._fernet is None:
            Fernet, _InvalidToken, _hashes, _PBKDF2HMAC = self._crypto()
            self._fernet = Fernet(self._load_or_create_key(self._password))
        return self._fernet
    ...
    def _invalid_token_type(self):
        _Fernet, InvalidToken, _hashes, _PBKDF2HMAC = self._crypto()
        return InvalidToken
    ...
    def _derive_key_from_password(self, password: str, salt: bytes, iterations: int) -> bytes:
        _Fernet, _InvalidToken, hashes, PBKDF2HMAC = self._crypto()
        kdf = PBKDF2HMAC(
